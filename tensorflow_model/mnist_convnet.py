...
def export_model(input_node_names, output_node_name):
    freeze_graph.freeze_graph(OUTPUT_DIR + '/' + MODEL_NAME + '.pbtxt', None, False,
                              OUTPUT_DIR + '/' + MODEL_NAME + '.chkp', output_node_name, "save/restore_all",
                              "save/Const:0", OUTPUT_DIR + '/frozen_' + MODEL_NAME + '.pb', True, "")

    input_graph_def = tf.GraphDef()

    with tf.gfile.Open(OUTPUT_DIR + '/frozen_' + MODEL_NAME + '.pb', "rb") as f2:
        input_graph_def.ParseFromString(f2.read())
    transfroms = ['quantize_weights']
    to_out = TransformGraph(input_graph_def, "input", output_node_name, transfroms)

    with tf.gfile.FastGFile(OUTPUT_DIR + '/trans_' + MODEL_NAME + '.pb', "wb") as f3:
